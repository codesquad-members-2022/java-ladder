...
public class InputValidator {
    ...
    public String[] names(String input) throws IllegalArgumentException {
        String[] names = input.replaceAll(" ", "").split(",");
        if (names.length < LadderGame.MIN_PLAYER) {
            throw new IllegalArgumentException("이름을 쉼표로 구분하여 " + LadderGame.MIN_PLAYER + "명 이상 입력해주세요.");
        }
        return names;
    }

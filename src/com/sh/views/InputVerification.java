...
public class InputVerification {
	...
	public static final String OUTPUT_RESULT_OF_GAME = "실행 결과를 입력하세요. (결과는 쉼표(,)로 구분하세요)";
	...
	public static final String ERROR_NUMBER_OF_RESULTS_ENTERED = "입력한 사람이름 수만큼 결과값이 동일해야 합니다.";
	...
	public static Predicate<String> isValidLength = (name) -> (name.length() > MAX_LENGTH_OF_NAME);
	...
	public static void isValidNames(String inputValues) {
		...
		isValidLengthOfEachWords(names);
	}

	public static void isValidResultInfo(String inputValues, int numberOfPeoples) {
		String[] results = getSplit(inputValues);
		isWhiteSpace(inputValues);
		isRulesOfInput(inputValues);
		isMatchNumberOfResults(results, numberOfPeoples);
		isValidLengthOfEachWords(results);
	}

	private static void isMatchNumberOfResults(String[] results, int numberOfPeoples) {
		if (results.length != numberOfPeoples) {
			throw new IllegalArgumentException(ERROR_NUMBER_OF_RESULTS_ENTERED);
		}
	}
	...
	private static void isValidLengthOfEachWords(String[] words) {
		Arrays.stream(words)
			.map(String::trim)
			.filter(word -> isValidLength.or(isStringBlank).test(word))
			.findAny()
